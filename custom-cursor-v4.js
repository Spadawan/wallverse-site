(() => {
  ...
  let effectsLevel = document.documentElement.dataset.effects || 'full';
  ...
  document.documentElement.classList.toggle('has-wallverse-cursor', effectsLevel !== 'low');
  cursor.hidden = effectsLevel === 'low';
  canvas.hidden = effectsLevel !== 'full';
  ...
  window.addEventListener('wallverse:effects-change', (event) => {
    effectsLevel = event.detail?.level || 'full';
    const enabled = effectsLevel !== 'low';
    document.documentElement.classList.toggle('has-wallverse-cursor', enabled);
