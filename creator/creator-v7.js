(() => {
  const start = () => {
    ...
    const shouldInsertAdAfter = helpers.shouldInsertAdAfter || ((position) => position === 12 || (position > 12 && (position - 12) % 15 === 0));
    ...
    const render = () => {
      ...
      cardsToShow.forEach((record, index) => {
        ...
        if (shouldInsertAdAfter(index + 1) && createAdCard) gridItems.push(createAdCard());
      });
