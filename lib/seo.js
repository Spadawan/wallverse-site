...
export async function themedPage(context, { title, description, path, body, schema, schemaId = 'seo-page-schema', noindex = false, image = `${ORIGIN}/assets/app-showcase.webp` }) {
  ...
  const rewritten = new HTMLRewriter()
    ...
    .on('head', { element(e) { e.append(`<link rel="stylesheet" href="/seo-pages.css?v=2">${schema ? `<script type="application/ld+json" id="${escapeHtml(schemaId)}">${jsonLd(schema)}</script>` : ''}`, { html: true }); } })
...
export async function catalogPage(context, category = '') {
  try {
    ...
    const cards = rows.map((w,i)=>{ const tier = cardTier(w); const frame = frames.get(String(w.id)) || 'default'; const polished = w.polished_until && new Date(w.polished_until) > new Date(); return `<a class="wallpaper" href="${escapeHtml(wallpaperPath(w))}"><article class="collectible-card collectible-card--public seo-wallpaper-card tier--${tier}${polished ? ' is-polished' : ''}${frame !== 'default' ? ` frame--${frame}` : ''}" data-card-frame="${frame}"><div class="collectible-card__media"><img class="collectible-card__image" src="${escapeHtml(thumbnail(w))}" alt="${escapeHtml(w.title)} phone wallpaper" width="360" height="800" loading="${i < 4 ? 'eager' : 'lazy'}" decoding="async"><div class="collectible-card__surface"></div><div class="collectible-card__shine"></div><div class="collectible-card__badges"><span class="tag">${escapeHtml(w.category || 'Wallpaper')}</span></div><div class="collectible-card__info"><h2>${escapeHtml(w.title)}</h2>${w.width && w.height ? `<p><span class="material-symbols-rounded" aria-hidden="true">high_quality</span>${Number(w.width)} × ${Number(w.height)}</p>` : ''}</div></div>${frameSkin(frame)}</article></a>`; }).join('');
