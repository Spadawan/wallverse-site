(() => {
  ...
  function makeSkin(frame) {
    ...
    skin.className = 'card-frame-skin'; skin.setAttribute('aria-hidden', 'true'); skin.dataset.frame = frame;
    for (let index = 1; index <= 2; index += 1) { const node = document.createElement('i'); node.className = `card-frame-skin__tracer card-frame-skin__tracer--${index}`; skin.append(node); }
    for (const position of ['top', 'right', 'bottom', 'left']) { const node = document.createElement('i'); node.className = `card-frame-skin__rail card-frame-skin__rail--${position}`; skin.append(node); }
    for (let index = 1; index <= 8; index += 1) { const node = document.createElement('i'); node.className = `card-frame-skin__spark card-frame-skin__spark--${index}`; skin.append(node); }
    for (let index = 1; index <= 4; index += 1) { const node = document.createElement('i'); node.className = `card-frame-skin__plate card-frame-skin__plate--${index}`; skin.append(node); }
    return skin;
  }
  ...
  function apply(card, ...values) {
    ...
    if (frame !== 'default') { card.classList.add(`frame--${frame}`); card.append(makeSkin(frame)); }
    return frame;
  }
  ...
  function applyAvatar(avatar, frameValue, rarity = 'common') {
    ...
    avatar.classList.toggle('avatar-frame', frame !== 'default'); avatar.dataset.avatarFrame = frame; avatar.dataset.avatarRarity = rarity;
    if (frame === 'default') {
      const previousContent = avatar.querySelector(':scope > .avatar-frame__content');
      if (previousContent) { while (previousContent.firstChild) avatar.insertBefore(previousContent.firstChild, previousContent); previousContent.remove(); }
      return frame;
    }
    avatar.classList.add(`avatar-frame--${frame}`, `avatar-tier--${rarity}`);
    let content = avatar.querySelector(':scope > .avatar-frame__content');
    if (!content) { content = document.createElement('span'); content.className = 'avatar-frame__content'; while (avatar.firstChild) content.append(avatar.firstChild); avatar.append(content); }
    return frame;
  }
