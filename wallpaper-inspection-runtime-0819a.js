(() => {
  ...
  const downloadAdPlacement = downloadDialog.querySelector('.download-ad-dialog__placement');
  ...
  function initializeDownloadAd() {
    ...
    const mountBanner = window.WallverseAds?.mountBanner;
    if (typeof mountBanner !== 'function') {
      console.warn('Download advertisement runtime is unavailable.');
      return;
    }
  ...
  async function confirmDownload() {
    ...
    const url = helpers.downloadUrl(wallpaper);
    if (!url) { downloadStatus.textContent = 'HD download is unavailable for this wallpaper.'; return; }
    downloadConfirm.disabled = true;
    // Count the confirmed action immediately. Waiting for fetch(...).blob() on
    // a large HD file meant guest downloads were often recorded too late.
    const downloadTask = triggerDirectDownload(url, downloadFileName(wallpaper, url));
    await recordDownload(wallpaper);
    downloadStatus.textContent = 'Preparing your HD download\u2026';
    await downloadTask;
