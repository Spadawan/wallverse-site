(() => {
  ...
  function celebrateLike() {
    ...
    for (let index = 0; index < 12; index += 1) {
      ...
      particle.textContent = index % 3 ? 'â™¥' : 'âœ¦'; likeButton.append(particle);
  ...
  function refreshInspectionStats() {
    document.getElementById('inspection-card-stats').replaceChildren(
      stat('â™¥', 'Likes', currentWallpaper.likes_count),
      stat('â‡©', 'Downloads', currentWallpaper.downloads_count),
  ...
  function renderWallpaper(wallpaper) {
    ...
    const frameLabel = frame === 'default' ? '' : ` Â· ${window.WallverseCardFrames.label(frame)}`;
    document.getElementById('inspection-rarity').textContent = `${tier}${frameLabel}${polished ? ' Â· Polished' : ''}`;
    document.getElementById('inspection-card-stats').replaceChildren(stat('â™¥', 'Likes', wallpaper.likes_count), stat('â‡©', 'Downloads', wallpaper.downloads_count), stat('visibility', 'Views', wallpaper.views_count));
    ...
    const facts = [
      ['Rarity', tier], ['Quality', helpers.qualityLabel(wallpaper.quality) || wallpaper.quality || 'â€”'],
      ['Dimensions', wallpaper.width && wallpaper.height ? `${wallpaper.width} Ã— ${wallpaper.height}` : 'â€”'],
      ['File size', formatBytes(wallpaper.file_size) || 'â€”'], ['Published', formatDate(wallpaper.created_at) || 'â€”'],
    ];
  ...
  function startDownloadCountdown() {
    ...
    downloadStatus.textContent = `Download available in ${remaining}â€¦`;
    downloadTimer = window.setInterval(() => {
      remaining -= 1;
      if (remaining > 0) { downloadStatus.textContent = `Download available in ${remaining}â€¦`; return; }
      clearDownloadCountdown();
  ...
  async function confirmDownload() {
    ...
    downloadStatus.textContent = 'Thanks for supporting Wallverse â¤ï¸';
  }
  async function loadComments(wallpaperId) {
    const list = document.getElementById('inspection-comment-list'); list.replaceChildren();
    const loading = document.createElement('p'); loading.textContent = 'Loading commentsâ€¦'; list.append(loading);
