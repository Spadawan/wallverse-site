(() => {
  ...
  const wallpaperFields = 'id,user_id,title,description,image_url,thumbnail_url,category,quality,width,height,file_size,likes_count,downloads_count,views_count,is_featured,is_weekly,polished_until,status,is_suggestive,created_at,storage_provider,thumbnail_storage_key,hd_storage_key,profiles!wallpapers_user_id_fkey(username,avatar_url,avatar_crop_x,avatar_crop_y,avatar_crop_scale,avatar_crop_rotation),wallpaper_tags(tags(name))';
  ...
  function setAvatar(node) {
    const username = profile?.username || 'W'; node.textContent = username.charAt(0).toUpperCase(); window.WallverseAvatarCrop?.apply(node, profile);
  ...
  async function ensureProfile(sessionUser) {
    const { data, error } = await client.from('profiles').select('id,username,role,avatar_url,avatar_crop_x,avatar_crop_y,avatar_crop_scale,avatar_crop_rotation,banner_url,followers_count,avatar_frame_type').eq('id', sessionUser.id).maybeSingle();
    if (error) throw error;
    if (data) return data;
    ...
    const { data: created, error: createdError } = await client.from('profiles').select('id,username,role,avatar_url,avatar_crop_x,avatar_crop_y,avatar_crop_scale,avatar_crop_rotation,banner_url,followers_count,avatar_frame_type').eq('id', sessionUser.id).single();
  ...
  async function load(sessionUser) {
    ...
    try {
      ...
      try {
        ...
        document.getElementById('profile-power').textContent = 'â€”';
      }
  ...
  document.getElementById('collection-suggestive').addEventListener('change', async (event) => {
    ...
    updateCollectionControls();
    status.textContent = 'Refreshing collectionâ€¦'; status.hidden = false;
