(() => {
  ...
  async function ensureProfile(sessionUser) {
    const { data, error } = await client.from('profiles').select('id,username,role,avatar_url,banner_url,followers_count,avatar_frame_type').eq('id', sessionUser.id).maybeSingle();
    if (error) throw error;
    if (data) return data;
    ...
    const { data: created, error: createdError } = await client.from('profiles').select('id,username,role,avatar_url,banner_url,followers_count,avatar_frame_type').eq('id', sessionUser.id).single();
