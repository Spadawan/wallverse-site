...
const ADSTERRA_GRID_BANNER_OPTIONS = Object.freeze({
  key: '9b67f852da5d9a4dc099ed07a1153c7d',
  format: 'iframe',
  height: 300,
  width: 160,
  params: {},
});
const ADSTERRA_DOWNLOAD_BANNER_OPTIONS = Object.freeze({
...
function makeAdsterraBannerResponsive(host, options) {
  ...
  const sync = () => {
    ...
    if (!iframe) return;
    const scale = Math.min(1, Math.max(0, host.clientWidth / options.width));
    host.style.setProperty('--ad-banner-scale', String(scale));
    host.style.setProperty('--ad-banner-width', `${options.width}px`);
    host.style.setProperty('--ad-banner-height', `${Math.round(options.height * scale)}px`);
    host.style.setProperty('--ad-banner-intrinsic-height', `${options.height}px`);
