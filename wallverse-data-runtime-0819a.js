...
const ADSTERRA_BANNER_URL = 'https://www.highperformanceformat.com/fe410c0ec832e31de5aec22aa9ee26d1/invoke.js';
const ADSTERRA_NATIVE_URL = 'https://pl30782857.effectivecpmnetwork.com/522b24f46ec68fe89a546004b33ecee5/invoke.js';
const ADSTERRA_NATIVE_CONTAINER_ID = 'container-522b24f46ec68fe89a546004b33ecee5';
const ADSTERRA_BANNER_OPTIONS = Object.freeze({
  key: 'fe410c0ec832e31de5aec22aa9ee26d1',
  format: 'iframe',
  height: 250,
  width: 300,
  params: {},
...
const publicCardsByWallpaperId = new Map();
...
function applyAvatarCrop(avatar, profile) {
  ...
  // Preserve its original centred rendering rather than applying user-avatar crop coordinates.
  const username = String(profile?.username || '').trim().replace(/^@/, '').toLowerCase();
  if (username === 'wallverse') return;
  const number = (value, fallback) => Number.isFinite(Number(value)) ? Number(value) : fallback;
  const limit = (value, minimum, maximum) => Math.min(maximum, Math.max(minimum, value));
  const cropX = limit(number(profile?.avatar_crop_x, 0), 0, 1);
  const cropY = limit(number(profile?.avatar_crop_y, 0), 0, 1);
  const cropScale = limit(number(profile?.avatar_crop_scale, 1), .05, 1);
  const rotation = number(profile?.avatar_crop_rotation, 0);
  avatar.classList.add('avatar--cropped');
  avatar.style.setProperty('--avatar-crop-width', `${100 / cropScale}%`);
  avatar.style.setProperty('--avatar-crop-offset-x', `${-cropX * 100}%`);
  avatar.style.setProperty('--avatar-crop-offset-y', `${-cropY * 100}%`);
  avatar.style.setProperty('--avatar-crop-rotation', `${rotation}rad`);
}
...
function creatorNode(profile, rarity = 'common', { framed = false } = {}) {
  ...
  applyAvatarCrop(avatar, profile);
...
function cardOwnerProfileFor(wallpaper) {
  return wallpaper?.web_card_owner_profile || wallpaper?.profiles || null;
}
...
function decorateCardFrame(wallpaper) {
  if (wallpaper) {
    const publicCard = publicCardsByWallpaperId.get(String(wallpaper.id || ''));
    wallpaper.web_card_frame_type = cardFrameFor(wallpaper);
    wallpaper.web_card_owner_profile_id = publicCard?.ownerProfileId || null;
    wallpaper.web_card_owner_profile = publicCard?.ownerProfileId ? (publicProfilesById.get(publicCard.ownerProfileId) || null) : null;
  }
  return wallpaper;
}
...
function renderCard(wallpaper) {
  ...
  const creator = creatorNode(cardOwnerProfileFor(wallpaper));
...
function makeAdsterraBannerResponsive(host) {
  if (!host || host.dataset.responsiveBanner === 'true') return;
  host.dataset.responsiveBanner = 'true';
  host.classList.add('is-banner');
  const sync = () => {
    const iframe = host.querySelector('iframe');
    if (!iframe) return;
    const scale = Math.min(1, Math.max(0, host.clientWidth / ADSTERRA_BANNER_OPTIONS.width));
    host.style.setProperty('--ad-banner-scale', String(scale));
    host.style.setProperty('--ad-banner-height', `${Math.round(ADSTERRA_BANNER_OPTIONS.height * scale)}px`);
    iframe.width = ADSTERRA_BANNER_OPTIONS.width;
    iframe.height = ADSTERRA_BANNER_OPTIONS.height;
    iframe.style.width = `${ADSTERRA_BANNER_OPTIONS.width}px`;
    iframe.style.height = `${ADSTERRA_BANNER_OPTIONS.height}px`;
  };
  const observer = new MutationObserver(sync);
  observer.observe(host, { childList: true, subtree: true });
  if ('ResizeObserver' in window) new ResizeObserver(sync).observe(host);
  sync();
}

function mountAdsterraBanner(host, onError = () => {}) {
  ...
  const request = adsterraBannerQueue.then(() => new Promise((resolve) => {
    ...
    window.atOptions = { ...ADSTERRA_BANNER_OPTIONS, params: {} };
    makeAdsterraBannerResponsive(host);
...
function loadAdsterraBanner(host, card) {
  return mountAdsterraBanner(host, () => collapseAdCard(card));
}

function loadAdsterraNative(host, card) {
  // The native provider requires one fixed container id. A rerender replaces
  // the old card, so allow the next live wide card to claim that placement.
  if (nativePlacementHost?.isConnected) {
    loadAdsterraBanner(host, card);
    return;
  }
...
function renderAdCard(variant = 'grid') {
  ...
  slot.className = 'ad-card__placement';
  slot.setAttribute('aria-label', 'Advertisement');

  card.append(label, slot);
  if (variant === 'wide') loadAdsterraNative(slot, card);
  else loadAdsterraBanner(slot, card);
  return card;
}
...
async function hydratePublicCardFrames() {
  ...
  publicCardsByWallpaperId.clear();
  publicProfilesById.clear();
  let offset = 0;
  try {
    while (true) {
      const rows = await fetchPublic(PUBLIC_CARD_FRAMES_VIEW, {
        select: 'wallpaper_id,card_id,frame_type,owner_profile_id',
      ...
      for (const row of rows) {
        ...
        // first stable public card so every visitor sees a consistent current
        // owner without exposing the private inventory relationship.
        if (!wallpaperId || publicCardsByWallpaperId.has(wallpaperId)) continue;
        publicCardsByWallpaperId.set(wallpaperId, { cardId: String(row?.card_id || ''), ownerProfileId: String(row?.owner_profile_id || '') || null, frame });
        if (frame !== 'default') publicFramesByWallpaperId.set(wallpaperId, frame);
      }
      if (rows.length < FEED_CATALOG_PAGE_SIZE) break;
      offset += rows.length;
    }
    const ownerIds = [...new Set([...publicCardsByWallpaperId.values()].map((card) => card.ownerProfileId).filter(Boolean))];
    for (let index = 0; index < ownerIds.length; index += 60) {
      const ids = ownerIds.slice(index, index + 60);
      const profiles = await fetchPublic('profiles', { select: 'id,username,avatar_url,avatar_crop_x,avatar_crop_y,avatar_crop_scale,avatar_crop_rotation,avatar_frame_type,role', id: `in.(${ids.join(',')})` });
      profiles.forEach((profile) => publicProfilesById.set(String(profile.id), profile));
    }
...
async function loadCreatorSpotlight() {
  const profiles = await fetchPublic('profiles', {
    select: 'id,username,role,avatar_url,avatar_crop_x,avatar_crop_y,avatar_crop_scale,avatar_crop_rotation,avatar_frame_type,followers_count',
